...
donation$same <- ifelse(donation$district == donation$con_district, 1, 0)

comp <- donation %>% group_by(district, cycle) %>%
  summarize(count = n(),
            ind = sum(same==1),
            outd = sum(same==0),
...
comp$out.pct <- comp$outd/comp$count
comp$out.amt.pct <- comp$outd_cont/(comp$outd_cont + comp$ind_cont)
...
can start from using "donation.csv" dataset
```{r}
